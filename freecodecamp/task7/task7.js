...
$(document).ready(function () {
   var arr=[],topMin = $('.s_window').offset().top,topMax = topMin + $('.s_window').height();
    $(".s_sub").click(function() {
        var text = $(".s_txt").val();
        if(text!==""){
            arr.push(text);
            textRun(arr.length-1);
            $(".s_txt").val('');
        }
    });
    ...
    function textRun(n) {
        if (arr.length> 0) {
            var texts = "<p class='text"+n+"'>" + arr[n] + "</p>";
            $(".barrage").append(texts);
            moveTex("text"+n);
        }
    }
   function moveTex(tex) {
        var texLeft = $("."+tex).width(),texTop=Math.floor(Math.random() * (280) + (topMin+50));
        $("."+tex).css({top:texTop});
        $("."+tex).animate({left: "-" + texLeft + "px"}, 20000,function () {
            var k=parseInt(tex.replace(/text/, ""));
            $("."+tex).remove();
            textRun(k);
        });
    }
